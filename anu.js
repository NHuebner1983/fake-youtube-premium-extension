...
function load() {
    ...
    var doPremiumCleanup = () => {
        document.querySelectorAll('ytd-rich-item-renderer').forEach(item => {
            if ((!item.textContent.includes('views') && !item.textContent.includes('watching'))) { item.classList.add('hidden') };
        });
    };

    window.premiumCleanupTimer = setInterval(doPremiumCleanup, 1000);
